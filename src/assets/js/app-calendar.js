$(() => {
  ...
  const highlightedDays = ['2018-5-10', '2018-5-11', '2018-5-12', '2018-5-13', '2018-5-14', '2018-5-15', '2018-5-16'];
  const date = new Date();
  ...
  const generateTime = function (element) {
    let n = 0,
      min = 30,
      periods = [' AM', ' PM'],
      times = [],
    ...
    times = times.concat(times.slice(0).map(time => time.replace(periods[0], periods[1])));

    // console.log(times);
    $.each(times, (index, val) => {
  ...
  $('#calendar').fullCalendar({
    ...
    eventAfterAllRender(view) {
      if (view.name === 'listMonth' || view.name === 'listWeek') {
        const dates = view.el.find('.fc-list-heading-main');
        dates.each(function () {
    ...
    eventRender(event, element) {
      ...
      const eBorderColor = (event.source.borderColor) ? event.source.borderColor : event.borderColor;
  ...
  const azCalendar = $('#calendar').fullCalendar('getCalendar');
  ...
  azCalendar.option('windowResize', (view) => {
  ...
  const azDateNow = azCalendar.getDate();
  azCalendar.option('select', (startDate, endDate) => {
  ...
  azCalendar.on('eventClick', (calEvent, jsEvent, view) => {
    const modal = $('#modalCalendarEvent');
