$(() => {
  ...
  $('#azNavShow, #azNavbarShow').on('click', (e) => {
    e.preventDefault();
  ...
  $('#azContentLeftShow').on('click touch', (e) => {
    e.preventDefault();
    $('body').addClass('az-content-left-show');
  });

  // This will hide left content from showing up in mobile only
  $('#azContentLeftHide').on('click touch', (e) => {
    e.preventDefault();
    $('body').removeClass('az-content-left-show');
  });

  // this will hide content body from showing up in mobile only
  $('#azContentBodyHide').on('click touch', (e) => {
    e.preventDefault();
  ...
  $('.az-navbar-backdrop').on('click touchstart', () => {
  ...
  $(document).on('click touchstart', (e) => {
    ...
    const dropTarg = $(e.target).closest('.az-header .dropdown').length;
    if (!dropTarg) {
    ...
    if (window.matchMedia('(min-width: 992px)').matches) {
      const navTarg = $(e.target).closest('.az-navbar .nav-item').length;
      if (!navTarg) {
